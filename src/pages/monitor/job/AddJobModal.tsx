...
import { addJob } from './service';

export default function AddJobModal({ dictMaps }: { dictMaps: DictMaps }) {
  const { loading, submitHandle } = useSubmit<Job>(data =>
    addJob(data).then(() => {
      message.success('新增定时任务成功');
    }),
  );
  ...
  return (
    <Form<Job>
      ...
      initialValues={{ jobGroup: 'default', misfirePolicy: '3', concurrent: '1', status: '0' }}
