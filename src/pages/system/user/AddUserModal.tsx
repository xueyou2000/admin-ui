...
export default function AddUserModal({ deptId, currentUser }: { deptId?: number; currentUser: SystemUser }) {
  ...
  const { loading, submitHandle } = useSubmit<SystemUser>(data =>
    addUser(data).then(() => {
      message.success(intl.formatMessage({ id: 'UserPage.addSuccess', defaultMessage: '新增用户成功' }));
    }),
  );
  ...
  return (
    ...
    >
      <ModalContent>
        ...
        <Form.Item
          name="password"
          label={<FormattedMessage id="User.password" defaultMessage="密码" />}
          rules={[
            {
              required: true,
              message: intl.formatMessage({ id: 'User.password.required', defaultMessage: '请输入密码' }),
            },
          ]}
        >
          <Input
            type="password"
            placeholder={intl.formatMessage({ id: 'User.password.placeholder', defaultMessage: '登陆密码' })}
          />
        </Form.Item>
        <Form.Item
          name="userName"
